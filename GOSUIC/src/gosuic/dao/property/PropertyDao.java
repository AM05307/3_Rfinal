...
public class PropertyDao {
	...
	public List<PropertyVo> listApt(String addr) {
		...
		if (address.length == 3) {
			...
			list = getJdbcTemplate().query(
					...
					new RowMapper<PropertyVo>() {
						...
						public PropertyVo mapRow(ResultSet rs, int num) throws SQLException {
							...
							// int rent=
							// propertyservice.getMaxRent(rs.getString("sigungu"),rs.getString("bunji"),rs.getString("danji"),rs.getString("myunjuk"),rs.getString("floor"));
			...
			String addr1 = address[0];
			String addr2 = address[1];
			list = getJdbcTemplate().query("SELECT * FROM apt_sil where sigungu like '%" + addr1 + "%" + addr2 + "%'",
					new RowMapper<PropertyVo>() {
			...
			list = getJdbcTemplate().query("SELECT * FROM apt_sil where sigungu like '%" + addr + "%'",
					new RowMapper<PropertyVo>() {
	...
	public List<PropertyVo> listOffi(String addr) {
		...
		if (address.length == 3) {
			...
			String addr1 = address[0];
			String addr2 = address[1];
			list = getJdbcTemplate().query("SELECT * FROM offi_sil where sigungu like '%" + addr1 + "%" + addr2 + "%'",
					new RowMapper<PropertyVo>() {
			...
			System.out.println(addr);
			list = getJdbcTemplate().query("SELECT * FROM offi_sil where sigungu like '%" + addr + "%'",
					new RowMapper<PropertyVo>() {
	...
	public List<PropertyVo> listYeunda(String addr) {
		...
		if (address.length == 3) {
			...
			System.out.println(addr);
			list = getJdbcTemplate().query("SELECT * FROM Yeun_da_sil where sigungu like '%" + addr + "%'",
					new RowMapper<PropertyVo>() {
	...
	public List<PropertyVo> detailpropertysil(PropertyVo pv, String tName) {
		...
		detail = getJdbcTemplate().query("SELECT * FROM " + tName + "_sil where sigungu ='" + pv.getSigungu()
				+ "' and bunji='" + pv.getBunji() + "' and danji='" + pv.getDanji() + "' and myunjuk='"
				+ pv.getMyunjuk() + "' and floor='" + pv.getFloor() + "' ", new RowMapper<PropertyVo>() {
	...
	public List<PropertyVo> userAt_list(PropertyVo pv, String tName) {
		System.out.println("propertydao listapt()메소드");

		List<PropertyVo> list = null;
		String query ="SELECT * FROM "+tName+"_sil where sigungu='"+pv.getSigungu()+"' and bunji='"+pv.getBunji()+"' and danji='"+pv.getDanji()	+"' and myunjuk='"+pv.getMyunjuk()+"' and floor='"+pv.getFloor()+"'";
		System.out.println(query);

		list = getJdbcTemplate().query(query, new RowMapper<PropertyVo>() {
